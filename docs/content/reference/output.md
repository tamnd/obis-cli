...
describes what users get. Pick a format with `-o`, or let obis choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
obis <command> -o table   # aligned columns for reading
obis <command> -o jsonl   # one JSON object per line, for piping
obis <command> -o json    # a single JSON array
obis <command> -o csv     # spreadsheet friendly
obis <command> -o tsv     # tab-separated
obis <command> -o url     # just the URL column
obis <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
obis <command> --fields id,title,url
```
...
```bash
obis <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
obis <command>            # a table, because this is a terminal
obis <command> | wc -l    # JSONL, because this is a pipe
```
